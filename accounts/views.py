    ...
    def get(self, request):
        ...
        if request.user.is_authenticated:
            if request.user.is_superuser:
                return redirect("core:admin-profile")
            else:
                # return redirect("core:myprofile")
                print("user.....")
        return render(request, 'accounts/landing-page.html', context)
...
class RegisterView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            user.type = request.POST.get('type')  # Get the type from the POST data
            user.save()
            send_admin_mail(f"User Create", f"{ user.username } is created - ({ user.type })")
